...
class MyGradientSpan(val gradientWidth: Float, color: Int, highlightColor: Int) : CharacterStyle(),
    ...
    override fun updateDrawState(tp: TextPaint) {
        ...
        val o = 6 * ((runToLineMappings[runCount % runToLineMappings.size]) % (lineOffsets.size / 6))
        if (o != lastLineCount) {
            ...
            shader.setLocalMatrix(matrix.apply {
                ...
                reset()
                val overhangToRemove =
                    ((textLength + gradientWidth) * ourProgress - textLength).coerceIn(
                        0f,
                        gradientWidth - 1
                    )
